//event status code: 'upcoming' and 'past'
const events = [
  {
    title: "Scratch Day Celebration `23",
    date: "27th May",
    time: "10AM Everyday",
    link: "",
    venue:
      "TBD.",
    contact: ["08052465145", "08060386628"],
    status: "past",
    description:
      "We're excited to be hosting our first ever Scratch Day. Join us for a day of celebrating kids coding! It promises to be fun and educative :)",
    image: "/images/scratch-day.png",
    id: "1",
  },
  {
    title: "Easter Holiday Bootcamp `23",
    date: "13th April - 14th April",
    time: "10AM Everyday",
    link: "",
    venue:
      "Opposite Evangelical Nursery and Primary School, Lobi Quaters, Old GRA, Makurdi.",
    contact: ["08052465145", "08060386628"],
    status: "past",
    description:
      "Join us for a free Easter holiday bootcamp. At this bootcamp your kids will learn about build games and animating stories using Scratch software.",
    image: "/images/easter-bootcamp.jpg",
    id: "2",
  },
  {
    title: "Summer Bootcamp `22",
    date: "15th August - 20th August",
    time: "9AM Everyday",
    link: "https://forms.gle/1sh74HzVK7eYUiQ6A",
    venue:
      "Opposite Evangelical Nursery and Primary School, Lobi Quaters, Old GRA, Makurdi.",
    contact: ["08052465145", "08060386628"],
    status: "past",
    description:
      "Enroll your kids in a free summer bootcamp. Where they can learn variouse skills using softwares like Scratch and also some basic programming language experience with HTML and Python.",
    image: "/images/summer.jpg",
    id: "3",
  },
  {
    title: "Children's Day 2022 - Children of Mary Orphanage, Otukpo",
    date: "Children's day 2022",
    time: "",
    link: "",
    venue: "Children of Mary Orphanage, Otukpo",
    contact: [],
    status: "past",
    description: "",
    image: "/images/teen-code/3.jpg",
    id: "4",
  },
  {
    title: "Children's Day 2022 - Government College, Makurdi",
    date: "Children's day 2022",
    time: "",
    link: "",
    venue: "Government College, Makurdi",
    contact: [],
    status: "past",
    description: "",
    image: "images/teen-code/5.jpg",
    id: "5",
  },
  {
    title: "TIVID Orphanage Code Camp for Children's day 2020",
    date: "Children's day 2020",
    time: "",
    link: "",
    venue:
      "Opposite Evangelical Nursery and Primary School, Lobi Quaters, Old GRA, Makurdi.",
    contact: [],
    status: "past",
    description: "",
    image: "/images/teen-code/9.jpg",
    id: "6",
  },
];

export default events;
